...
export class CreateRideComponent {
  ...
  @ViewChild('waypointLocationRef') waypointLocationInput:| ElementRef| undefined;
...
place:{lat:number, lng:number}
  ...
  defaultTime(){
    const hour:number = 23,minutes:number= 59
    return `${hour}:${minutes}`;
  }
  ...
  onCountrySelect(event: any) {
    ...
    console.log(this.Rideform.get('phoneNumber').value && this.Rideform.get('phoneNumber').value.trim().length == 10);
    if(this.Rideform.get('phoneNumber').value && this.Rideform.get('phoneNumber').value.trim().length == 10){
      
      
      console.log("This is inside the TRUE condition");
      
      const confirm1 = confirm('Are you sure you want to find User Again ? Doing this you have to follow all the steps from starting again !')
      if(!confirm1){
  ...
  onSubmit() {
    ...
    this.createRideService
      .isUser('/is_user', this.selectedCountryId, this.Rideform.get('phoneNumber').value)
      .subscribe({
        next: (res: any) => {
  ...
  isLocationInside(data:{lat:number,lng:number}):any {
    this.createRideService.isLocationInsideZone('/isLocationInsideZone',data).subscribe({
      next:(res:any)=>{
  ...
  handlePlaceChange(place: any, caller: string) {
    ...
    if (caller === 'from') {
      ...
      const senddata = {lat:place.geometry.location.lat(),lng:place.geometry.location.lng()}
  ...
  addWaypoint() {
    ...
    const waypointValue = this.waypointLocationInput?.nativeElement.value;
    if (waypointValue.trim() === '' || (this.selectedWaypointLocation !== waypointValue)) {
      this.toaster.warning('Please select Waypoint from suggesstion', 'Warning');
      return;
    }
...
this.directionsService.route(
{
  ...
  waypoints: this.waypointsArray.map((waypoint: any) => ({
    location: waypoint, stopover: true
    // location: waypoint
  })),
...
//on isUser
 ...
 bookDateandTime: number;
  ...
  Calculate() {
    ...
    if(!this.isInsideZone ){
      this.toaster.warning("Service is not available at this 'From' location","! Invalid Input")
      return null
    }
    if((this.selectedToLocation !== toValue)|| (this.selectedfromLocation !== fromValue) ){
      this.toaster.warning("Please select From: and To: from Auto-Suggestion","! Invalid Input")
      return null
    }
    // console.log(fromValue, toValue, this.waypointLocation);

    
    if(!this.isInsideZone || (fromValue === toValue && (fromValue === this.waypointsArray[0] || toValue === this.waypointsArray[this.waypointsArray.length-1] ))){
      this.toaster.warning("Please select appropriate Location","! Invalid From location")
      return;
      }
...
ServicePricing:any[any]=[]
    ...
    this.createRideService.getPricingData('/get_pricing',data).subscribe({
      next:(res:any)=>{
        ...
        if(res.success === 'true' && res.data.length > 0){
          ...
          const result = RouteArray.reduce((acc:any, trip:any) => {
            return {
          ...
          const servicePricing = allPricingArray.map(({vehicleData,distanceBasePrice,pricePerUnitDistance,pricePerUnitTime,basePrice,minFare}) => {
            return {
              vehicleid : vehicleData._id,
              vehiclename: vehicleData.vname ,
              Totalprice : this.TotalPrice(Totaldistance,TotalTime,distanceBasePrice,pricePerUnitDistance,pricePerUnitTime,basePrice,minFare)
          ...
          this.toaster.info("No service Type available for this City","Sorry for the inconvinience")
          return;
  ...
  TotalPrice(distance:number,duration:number, distanceforbaseprice:number,Unitdistanceprice:number,Unittimeprice:number,baseprice:number,minFare:number){
    ...
    let TotalPricing = (tempdistance * Unitdistanceprice) + (TotalTime * Unittimeprice) + baseprice;
...
// assigning main ride details objects ( 4 )
  ...
  //   this.selectedOption = this.BookingrideForm.get('bookingType').value
  //   const currentDate = new Date();
    
  //   if (this.selectedOption === "bookNow") {
  //     // console.warn("------------------------BOOKNOW bro---------------");
  ...
  //     this.isDateandTimeValid = true;
  //   } else {
  //     this.BookingrideForm.get('date').enable();
  //     this.BookingrideForm.get('time').enable();
  ...
  //       this.BookingrideForm.get('date').value + "T" + this.BookingrideForm.get('time').value
  //     );
  ...
  //       this.isDateandTimeValid = false;
  //     }
  //     this.isDateandTimeValid = selectedDateTime > currentDate;
  //     if(!this.isDateandTimeValid ){
  //       console.log(" Error -> selectedDateTime",selectedDateTime);
  //       // this.selectedTime =this.formatTime(currentDate)
  //       this.toaster.error("Please choose valid Date and Time","Invalid Input");
  //       // this.BookingrideForm.setValue(this.selectedTime)
  //       return
  //     }
  //     console.log("isDateandTimeValid--->>>>>>>", this.isDateandTimeValid)
  //     // ------------------------------------------------------------------------------------------------------------------------------------------------//
  //   }
  // }
  ...
  onchangeDateandTime(){
    ...
    if (value === 'schedule') {
      ...
      //     } else {
      //       scheduleDateTimeControl?.clearValidators();
      //     }
  }else{
    DateTime?.clearValidators();
    // this.BookingrideForm.get('DateTime')
  }
  // const DateTime = this.BookingrideForm.get('DateTime')
  DateTime?.updateValueAndValidity()
}
//   this.BookingrideForm.get('bookingType')?.valueChanges.subscribe(value => {
//     const scheduleDateTimeControl = this.BookingrideForm.get('scheduleDateTime');
//     if (value === 'schedule') {
//       scheduleDateTimeControl?.setValidators([Validators.required, this.futureDateValidator]);
//     } else {
//       scheduleDateTimeControl?.clearValidators();
//     }
//     scheduleDateTimeControl?.updateValueAndValidity();
//   });
// }
...
console.log('Time:', timeString); // Output: 12:30:00
  ...
  findVehicleById(vehicleid:any) {
    return this.ServicePricing.find(vehicle => vehicle.vehicleid === vehicleid);
  }
...
onBackPage(){
  ...
  onSubmitRide(): void {
    ...
    if(this.BookingrideForm.invalid ){
      this.toaster.error("Please complete the all Form Details with Validations","Invalid Form")
      return
    }
    ...
    if(this.BookingrideForm.get('bookingType').value === 'bookNow'){
      ...
      console.log('this.BookingrideForm.getvalue',this.BookingrideForm.get('DateTime').value);
      
      if(this.BookingrideForm.get('DateTime').value){

        const [currentDate, currentTime] = this.BookingrideForm.get('DateTime').value.split('T');
        this.bookDate = currentDate;
        this.bookTime = currentTime;
        
        this.bookDateandTime = new Date(`${this.bookDate}T${this.bookTime}`).getTime();
         confirmation = confirm('Are you sure You have Selected this Date : '+ this.bookDate+' '+ this.bookTime);
    ...
    if(this.fromLocation !== fromValue || this.toLocation !== toValue ){
      this.toaster.error("Please Enter Valid Ride Details or Calculate Again","Invalid Ride Details")
  
      return
    }
    // const confirmation = confirm('Are you sure You have Selected this Date : '+this.BookingrideForm.get('date').value+ ' and Time : '+this.BookingrideForm.get('time').value);
